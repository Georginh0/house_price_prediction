...
class OutlierDetectionStrategy(ABC):
    ...
    def detect_outliers(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Parameters:
        df (pd.DataFrame): The dataframe containing features for outlier
        detection.

        Returns:
        pd.DataFrame: A boolean dataframe indicating where outliers are
        located.
...
if __name__ == "__main__":
    ...
    # # Initialize the OutlierDetector with the Z-Score based Outlier
    # Detection Strategy
    ...
    outlier_detector.visualize_outliers(
        df_cleaned, features=["SalePrice", "Gr Liv Area"]
    )
    pass
